...
export default function CategoryBudgets() {
  ...
  return <main className="cb-screen">
    ...
    {scope === 'group' ? <>
      ...
      {activeGroup.members.map(member => <div key={member.id} className="cb-category cb-member">
        <div className="cb-category-head">
          ...
          <span className={member.oweCents ? 'cb-owe' : ''}>
            {member.paidCents !== undefined ? `${formatCents(member.paidCents)} paid` : `Owes ${formatCents(member.oweCents ?? 0)}`}
          </span>
        </div>
      ...
      <div className="cb-notice cb-settle" role="status">
        <strong>{activeGroup.settleUp.title}</strong>
